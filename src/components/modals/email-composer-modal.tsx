...
export default function EmailComposerModal({
  ...
  const recipientDescription = useMemo(() => {
    ...
    if (recipientNames.length === 2)
      return `${recipientNames[0]} and ${recipientNames[1]}`;
  ...
  const handleSaveTemplate = async () => {
    ...
    if (!subject.trim() || !body.trim()) {
        toast({ title: 'Subject and body are required to save a template', variant: 'destructive' });
        return;
    }
    setIsLoading(true);
    try {
  ...
  const handleSendCertificate = async () => {
    if (recipientEmails.length !== 1) {
      toast({
        ...
        description: 'Certificates can only be sent to one user at a time.',
      });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: `A Bug Finder certificate has been sent to ${recipientNames[0]}.`,
        });
        onClose();
      } else {
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              . You can start from a template, preview your message, and save it as
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Enter a name for this email template. It will be available for
              future use.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleSaveTemplate} disabled={isLoading || !newTemplateName}>
              {isLoading ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : null}
              Save Template
